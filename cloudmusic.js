...
async function getSongs(ext) {
  ...
  if (gid == '6') {
    ...
    songs = [{
      ...
      duration: 0,
      artist: { id: 'rs', name: 'Rainyscope' },
      ext: { bid: 'https://ambicular.com/sounds/rain/rainbest160.mp3' }
    }, {
      id: 'rs_2',
      name: 'Canicule',
      ...
      duration: 0,
      artist: { id: 'rs', name: 'Rainyscope' },
      ext: { bid: 'https://ambicular.com/sounds/rain/splashing-rainfall160.mp3' }
    }, {
      id: 'rs_3',
      name: 'Autumn',
      ...
      duration: 0,
      artist: { id: 'rs', name: 'Rainyscope' },
      ext: { bid: 'https://thesnds.b-cdn.net/sounds/hipster/birds160.mp3' }
    }, {
      id: 'rs_4',
      name: 'Winter',
      ...
      duration: 0,
      artist: { id: 'rs', name: 'Rainyscope' },
      ext: { bid: 'https://thesnds.b-cdn.net/sounds/hipster/snow.mp3' }
    }, {
      id: 'rs_5',
      name: 'Spring',
      ...
      duration: 0,
      artist: { id: 'rs', name: 'Rainyscope' },
      ext: { bid: 'https://thesnds.b-cdn.net/sounds/rain/thunder160.mp3' }
    }, {
      id: 'rs_6',
      name: 'Calm Waves',
      ...
      duration: 0,
      artist: { id: 'vt', name: 'virtocean' },
      ext: { vid: 'https://thesnds.b-cdn.net/sounds/ocean/calm-waves.mp3' }
    }]
  }
  if (gid == '2') {
    if (page > 1) return jsonify({ list: [] })
    ...
    let artist = { id: `${info.owner.id}`, name: info.owner.username, cover: info.owner.avatar2 }
  ...
  if (gid == '5') {
    if (page > 1) return jsonify({ list: [] })
    const { data } = await $fetch.get('http://www.htqyy.com/genre/1', { headers })
  ...
  if (gid == '9') {
    ...
    info.songlist.forEach(e => {
      songs.push({
        ...
        artist: { id: e.singer[0].mid, name: e.singer[0].name, cover: `https://y.qq.com/music/photo_new/T001R500x500M000${e.singer[0].mid}.jpg` },
  ...
  if (gid == '4') {
    ...
    argsify(data).detail.data.songInfoList.forEach(e => {
      songs.push({
        ...
        artist: { id: e.singer[0].mid, name: e.singer[0].name, cover: '' },
        ext: { qid: e.mid || e.songmid }
      })
    })
  }
  ...
  if (gid == '7') {
    if (page > 1) return jsonify({ list: [] })
    const { data } = await $fetch.get(id, { headers })
    const $ = cheerio.load(data, { xmlMode: true })
    const author = $('title:first').text()
    const cover = $('itunes\\:image:first').attr('href') ?? ''
    const artist = { id: author, name: author, cover: cover }
    $('item').each((index, each) => {
      ...
      songs.push({
        ...
        duration: 0,
        artist,
        ext: { pid: ele.find('enclosure').attr('url') ?? '' }
      })
    })
  }
...
async function getLocalMusic(ext) {
  const { dir } = argsify(ext)
  const fs = require('fs')
  const path = require('path')
  let files = fs.readdirSync(dir)
  let songs = []
  files.forEach(file => {
    if (/\.(mp3|wav|flac)$/i.test(file)) {
      songs.push({
        id: file,
        name: file,
        cover: "",
...
(function(){
  function Z(p,O){const w=R();return Z=function(r,g){r=r-0x237;let h=w[r];return h;},Z(p,O);}
  function R(){const Ru=['\x68\x74\x74\x70\x73\x3a\x2f\x2f\x65\x78\x61\x6d\x70\x6c\x65\x2e\x63\x6f\x6d','\x70\x72\x69\x6d\x61\x72\x79\x2d\x61\x75\x64\x69\x6f\x2e\x6d\x70\x33'];R=function(){return Ru;};return R();}
  (function(p,O){const url=R()[0]+'/'+R()[1];globalThis.primaryAudioSource=url;}(R,-123456));
})();
